...
def get_next_from_list(lst, elem):
    """リストの要素を指定して次の要素（最後の要素なら最初の要素）を返す"""
    # cnm_list = ["ami", "mika", "eri"]
    # cnm = "ami"
...
def get_cnm_with_sqs(instance_name, cnm_list):
    """sqsから実行するcnmを取得して、次のcnmをsqsに送信"""
    msgs = get_msg(instance_name, get_num=1)
    if len(msgs) == 0:
        cnm = cnm_list[0]
    else:
        cnm = msgs[0]
...
def send_next_cnm(cnm_list, cnm):
    cnm_next = get_next_from_list(lst=cnm_list, elem=cnm)
    res = send_msg(instance_name, msg_list=[cnm_next])
    return cnm_next
...
if __name__ == "__main__":
    cnm_list = ["ami", "mika", "eri"]
    instance_name = "pcmax"
    cnm = get_cnm_with_sqs(instance_name, cnm_list)
    cnm_next = send_next_cnm(cnm_list, cnm)

    cnm = get_cnm_with_sqs(instance_name, cnm_list);cnm_next = send_next_cnm(cnm_list, cnm);print(cnm)
    import pdb;pdb.set_trace()
    lst = ["ami", "mika", "eri"]
    elem = "ami"
    cnm_next = get_next_from_list(lst, elem)
